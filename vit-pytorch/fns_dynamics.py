...
if __name__ == '__main__':
    ...
    parser.add_argument('--is_eigvals', type=str2bool, nargs='?', const=True, default=False)
    ...
    for model_idx, model_dir in enumerate(model_dirs):        
        ...
        if model_idx == 0:
            ...
            if dataset_name == 'cifar10':
                #trainloader, testloader, _ = prepare_data(batch_size=N_batch, num_workers=ddp_world_size)    
                trainloader, testloader, _ = prepare_data(batch_size=N_batch, num_workers=1)
        ...
        lstyles = ['-', '--', '--']
        #axss[lidx][mr_ii].plot(idxs, eigvals, c=c_hyp, linestyle=lstyles[alphas.index(alpha)], label=r'$q$ kept')  # Non-uniform
        ...
        #print(attn_weights)                
        #axss[lidx][ax_idx].imshow(attn_weights.detach().numpy(), cmap=cmap_attn, norm=cmap_norm)  # attn_weights or K_tilde                

        # value_t0 = attention_probs.clone().detach().numpy() @ eigvecs
        # value_t1 = attention_probs.clone().detach().numpy() @ eigvecs

        if not args.is_eigvals:
            axss[lidx][ax_idx].scatter(eigvecs[:,-1], eigvecs[:,-2], c=c_hyp, s=2)        
        else:
            axss[lidx][ax_idx].plot(range(len(eigvals)), eigvals, c=c_hyp)
    ...
    for lidx in range(config['num_hidden_layers']):
        ...
        for mr_ii in range(len(axss[lidx])):
            if mr_ii < ncols:
                #axss[lidx][mr_ii].set_xscale('log') 
                #axss[lidx][mr_ii].set_yscale('log')    
                pass    
        ...
        if args.is_eigvals:
            fig_file += 'dynamics_evals-' 
        else:
            fig_file += 'dynamics_evecs-'
