...
if __name__ == '__main__':
    ...
    for eps_idx, eps in enumerate(epss):
        ...
        for model_idx, model_dir in enumerate(model_dirs):
            ...
            _, attention_weights_init, g_dists_init = model_init(X[None])
            ...
            for lidx, g_dist in enumerate(g_dists):
                ...
                if args.is_include_pad_mask:
                    ...
                    g_dist_init = g_dists_init[lidx][:,:,:X_len,:X_len]
                else:
                    g_dist = g_dist[:,:,:,:]
                    g_dist_init = g_dists_init[lidx][:,:,:,:]
                    X_len = config['max_len']                
                # print('g_dist')
                # print(g_dist)  # delete
                if 'v2_' not in manifold:
                    attn_score = dist_to_score(g_dist, alpha, bandwidth, d_intrinsic=d_intrinsic)
                    attn_score_init = dist_to_score(g_dist_init, alpha, bandwidth, d_intrinsic=d_intrinsic)
                else:
                    attn_score = dist_to_score(g_dist, alpha, bandwidth, d_intrinsic=1)
                    attn_score_init = dist_to_score(g_dist_init, alpha, bandwidth, d_intrinsic=1)
                ...
                if a > 0:            
                    ...
                    K_tilde = attn_score
                    K_tilde_init = attn_score_init

                K_tilde = K_tilde[0,0]
                D_tilde = K_tilde.sum(-1)
                K_tilde_init = K_tilde_init[0,0]
                D_tilde_init = K_tilde_init.sum(-1)

                attention_weight = F.normalize(K_tilde,p=1,dim=-1)  # can do this as the attn weights are always positive
                attention_weight_init = F.normalize(K_tilde_init,p=1,dim=-1)
                ...
                if config['qk_share']:
                    ...
                    eigvecs = eigvecs[:,ii]      
                    # ----- before training -----   
                    K_hat_init = torch.diag(D_tilde_init**(-0.5)) @ K_tilde_init @ torch.diag(D_tilde_init**(-0.5))
                    K_hat_sym_init = 0.5*(K_hat_init + K_hat_init.T)
                    eigvals_init, eigvecs_init = torch.linalg.eigh(K_hat_sym_init)    
                    eigvecs_init = torch.diag(D_tilde_init**(-0.5)) @ eigvecs_init

                    eigvals_init = eigvals_init.detach().numpy()
                    eigvecs_init = eigvecs_init.detach().numpy()      
                    # order based on eigvals from large to small
                    ii = np.argsort(eigvals_init)
                    eigvals_init = eigvals_init[ii]
                    eigvecs_init = eigvecs_init[:,ii]    
                ##### Q != K case #####
                else:
                ...
                # ----- Q = K -----
                    ...
                    # Spectral density
                    # after training
                    axs[alphas.index(alpha),0].scatter(np.arange(1,len(eigvals)+1), eigvals, c=c_hyp, s=1)
                    # before training
                    axs[alphas.index(alpha),1].scatter(np.arange(1,len(eigvals_init)+1), eigvals_init, c=c_hyp, s=1)

                    # axs[2,alphas.index(alpha)].set_xscale('log')
                    # axs[2,alphas.index(alpha)].set_yscale('log')

                    # label rows
                    axs[alphas.index(alpha),0].set_ylabel(rf'$\alpha$ = {alpha}', fontsize=10)    
                    axs[alphas.index(alpha),0].set_xlabel(rf'Eigenvalue index',fontsize=8)
                    #axs[alphas.index(alpha),0].set_ylabel(rf'Eigenvalue',fontsize=8)
                    axs[alphas.index(alpha),1].set_xlabel(rf'Eigenvalue index',fontsize=8)
                    #axs[alphas.index(alpha),1].set_ylabel(rf'Eigenvalue',fontsize=8)     
                    
                    # label columns
                    axs[0,0].set_title('After training',fontsize=8)
                    axs[0,1].set_title('Before training',fontsize=8)   

                    # log y-axis
                    axs[alphas.index(alpha),0].set_yscale('log'); axs[alphas.index(alpha),1].set_yscale('log')       
        ...
        fig_file = f'spectral_gap-{args.fns_type}-eps={eps}-pretrained_embd={pretrained_model_name}-{qk_affix}.pdf'
        plt.tight_layout()
