# TRAINER TRAINING IS NOT INTENDED TO FUNCTION; USE DDPMAIN
    ...
    def train(self, trainloader, testloader, epochs, save_model_every_n_epochs=0):
        ...
        for i in tqdm(range(epochs)):
            ...
            if (
                ...
                save_checkpoint(
                    self.exp_name, self.model, i + 1, base_dir=self.base_dir
                )
        # Save the experiment
        save_experiment(
            self.exp_name,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="translation/main.py training arguments"
    )
    ...
    parser.add_argument(
        "--model_root",
        default=njoin(DROOT, "trained_models"),
        type=str,
        help="root dir of storing the model",
    )
    ...
    parser.add_argument(
        "--model_name",
        default="dptranslation",
        type=str,
        help="dptranslation | fnstranslation",
    )
    ...
    # ---------- Device and system ----------
    dev = torch.device(
        f"cuda:{torch.cuda.device_count()}" if torch.cuda.is_available() else "cpu"
    )
    ...
    # ---------- Load dataset ----------
    if args.dataset_name == "iwslt14":
        trainloader, testloader, _ = prepare_data(batch_size=batch_size)  # CHANGE
    ...
    if args.model_root == "":
        model_root = structural_model_root(
            qk_share=args.qk_share,
            n_layers=args.n_layers,  # CHANGE
    ...
    # quit()  # delete
    trainer = MyTrainer(
        config, model, optimizer, loss_fn, args.exp_name, device=dev, base_dir=model_dir
    )
    trainer.train(
        trainloader,
        testloader,
