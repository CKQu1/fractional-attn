...
class DecoderBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.use_faster_attention = config.get("use_faster_attention", False)
        if self.use_faster_attention:
            self.self_attention = FasterSPOPFNSMultiHeadAttention(config)
            self.cross_attention = FasterSPOPFNSMultiHeadAttention(config, is_cross_attention=True)
        else:
            self.self_attention = SPOPFNSMultiHeadAttention(config)
            self.cross_attention = SPOPFNSMultiHeadAttention(config, is_cross_attention=True)
        self.layernorm_1 = nn.LayerNorm(config["hidden_size"])
        self.layernorm_2 = nn.LayerNorm(config["hidden_size"])
        self.mlp = MLP(config)
