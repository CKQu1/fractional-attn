...
class Attention(nn.Module):
    def __init__(self, config):
        ...
        if 'fns' not in self.attn_type:
            self.W_q = nn.Linear(self.dim, self.num_head * self.head_dim, bias=self.bias)
            self.W_k = nn.Linear(self.dim, self.num_head * self.head_dim, bias=self.bias)
            self.W_v = nn.Linear(self.dim, self.num_head * self.head_dim, bias=self.bias)

            self.attn = attn_selector(self.attn_type, config, self.W_q, self.W_k, self.W_v)
        else:            
            self.W_k = orthogonal(nn.Linear(self.dim, self.num_head * self.head_dim, bias=self.bias))
            self.W_v = nn.Linear(self.dim, self.num_head * self.head_dim, bias=self.bias)      

            #self.attn = attn_selector(self.attn_type, config, 1, self.W_k, self.W_v)              
            self.attn = attn_selector(self.attn_type, config, None, None, None)
