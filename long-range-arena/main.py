...
from train_helpers import estimate_loss, get_lr, get_batch
####################
...
if __name__ == '__main__':
    ...
    trainloader, valloader, testloader, n_classes, seq_len, in_dim, train_size = \
        create_dataset_fn(seed=args.seed, train_bs=train_batch_size, eval_bs=eval_batch_size)
    ...
    def estimate_loss():
        ...
        for split in ['train', 'val']:
            ...
            for k in range(eval_iters):
                X, Y, src_mask, src_pad_mask = get_batch(split)
                with ctx:                    
                    if retrieval:
                        logits, _ = model(X, Y, src_mask, src_pad_mask) # NOT REALLY LOGITS
                    else:
    ...
    t0 = time.time()   
    X, Y, src_mask, src_pad_mask = get_batch(train_iter, seq_len, in_dim, config["src_pad_token_id"])
    iter_num = 0 
    ...
    while True:
        ...
        if iter_num % eval_interval == 0 and master_process:
            ...
            print(f"step {iter_num}: train loss {losses['train']:.4f}, val loss {losses['val']:.4f}, val accuracy {acc['val']:.4f}")
            if wandb_log: # CHANGE
                wandb.log({
                    "iter": iter_num,
                ...
                metrics_ls.append([iter_num, lr, losses['train'].item(), losses['val'].item(), acc['val'], dt])
