...
class Manifold(object):
   ...
   # -----------------------------------------------------------------------
    def _smooth_and_rotate(self):
        ...
        smoothed_positionss, rotation_matricess = [], []
        for aidx, alpha in enumerate(self.alphas):
            assert 1 <= alpha <= 2, 'alpha must be in [1,2]'
            if alpha == 2:
                ...
                x_coord = stats.levy_stable.rvs(alpha=alpha, beta=0, loc=0, scale=np.sqrt(self.step_size),
                                                size = self.n_steps)
                y_coord = stats.levy_stable.rvs(alpha=alpha, beta=0, loc=0, scale=np.sqrt(self.step_size),
                                                size = self.n_steps)
            ...
            smoothed_positionss.append(smoothed_positions)
            rotation_matricess.append(rotation_matrices)

        return smoothed_positionss, rotation_matricess
    ...
    def simulate_brownian_sphere(self, manifold=None, plot=False):
        ...
        smoothpositionss, rotationmatriciess= self._smooth_and_rotate()
        ...
        if plot is True:
            self.plot_brownian_sphere(brownianspheres)

        return brownianspheres



    # -----------------------------------------------------------------------
    def plot_brownian_sphere(self, sphere_bms,
                                     manifold=None,
                                     surface_color= 'red',
                                     colorbar='viridis',
                                     marker='.',
        ...
        # elif isinstance(steptoplot,int):
        ...
        for i in range(len(self.alphas)):
            ...
            #timebar = np.arange(1,steptoplot[i]+1,1)
            timebar = np.arange(1,self.n_steps+1,1)
            brown = ax.scatter(sphere_bms[i][0:self.n_steps,0],
                               sphere_bms[i][0:self.n_steps,1],
                               sphere_bms[i][0:self.n_steps,2],
                               c=timebar, cmap=colorbar,
                               marker=marker,
                               s=markersize,
                               facecolor='0.5', 
            ...
            # ax.plot(sphere_bms[i][0:self.n_steps,0],
            #         sphere_bms[i][0:self.n_steps,1],
            #         sphere_bms[i][0:self.n_steps,2],
            #         c='k', linewidth=0.01)
        ...
        plt.savefig(os.path.join(save_dir, f'{self.manifold}_diffusion.pdf'))        
