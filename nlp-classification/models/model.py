...
from models.model_att import DPAttention, OPDPAttention, FNSAttention, SPFNSAttention, SPOPFNSAttention, RDFNSAttention, RDOPFNSAttention, SINKAttention, OPSINKAttention
...
class WrapperPreTrainedModel(PreTrainedModel):
    config_class = ModelConfig  # determines tokenizer
    base_model_prefix = "fnsformer"
...
class Model(WrapperPreTrainedModel):
    def __init__(self, config, add_pooling_layer=True, **kwargs):
        ...
        self.encoder = ModelEncoder(config, **kwargs)
    ...
    def forward(
        ...
        #print(f'shape: {self.get_extended_attention_mask(attention_mask, input_shape, device).shape}')
        #print(f'extended_attention_mask shape: {extended_attention_mask.shape}')  # delete
...
class ModelEncoder(nn.Module):
    def __init__(self, config, **kwargs):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([Block(config, layer_id=i,**kwargs) for i in range(config.num_hidden_layers)])
...
class Block(nn.Module):
    def __init__(self, config, layer_id=0, **kwargs):
        ...
        elif model_name == 'opsinkformer':
            self.attention = OPSINKAttention(config, layer_id, **kwargs)    
        elif model_name == 'dpformer':
            self.attention = DPAttention(config,  layer_id, **kwargs)
        elif model_name == 'opdpformer':
            self.attention = OPDPAttention(config,  layer_id, **kwargs)            
