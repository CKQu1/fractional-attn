...
class RDOPFNSAttentionHead(nn.Module):
    """
    A single attention head.
    This module is used in the RDOPFNSMultiHeadAttention module.
...
class RDOPFNSMultiHeadAttention(nn.Module):
    """
...
class FasterRDOPFNSMultiHeadAttention(nn.Module):
    """
...
class EncoderBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.use_faster_attention:
            self.attention = FasterRDOPFNSMultiHeadAttention(config)
        else:
            self.attention = RDOPFNSMultiHeadAttention(config)
...
class DecoderBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.use_faster_attention = config.get("use_faster_attention", False)
        if self.use_faster_attention:
            self.self_attention = FasterRDOPFNSMultiHeadAttention(config)
            self.cross_attention = FasterRDOPFNSMultiHeadAttention(config, is_cross_attention=True)
        else:
            self.self_attention = RDOPFNSMultiHeadAttention(config)
            self.cross_attention = RDOPFNSMultiHeadAttention(config, is_cross_attention=True)
        self.layernorm_1 = nn.LayerNorm(config["hidden_size"])
        self.layernorm_2 = nn.LayerNorm(config["hidden_size"])
        self.mlp = MLP(config)
