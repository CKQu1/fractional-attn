...
class Trainer:
    def __init__(self, args, train_loader, test_loader, tokenizer, n_it=3,
                 alpha=1.2, bandwidth=1, a=0):
        self.args = args
        ...
        if args.model_name.lower() == 'rdfnsformer':
            from models.rdfnsformer import RDFNSformer
            model = RDFNSformer(alpha       = alpha,
                                bandwidth   = bandwidth,
                                a           = a,
        ...
        elif args.model_name.lower() == 'spfnsformer':
            from models.spfnsformer import SPFNSformer
            model = SPFNSformer(alpha       = alpha,
                                bandwidth   = bandwidth,
                                a           = a,
                                vocab_size  = self.vocab_size,
                                seq_len     = args.max_seq_len,
        ...
        elif args.model_name.lower() == 'sinkformer':
            from models.sinkformer import SINKformer
            model = SINKformer(vocab_size  = self.vocab_size,
                               seq_len     = args.max_seq_len,
                               d_model     = args.hidden,
                               n_layers    = args.n_layers,
                               n_heads     = args.n_attn_heads,
        ...
        elif args.model_name.lower() == 'dpformer':
            from models.dpformer import DPformer
            model = DPformer(vocab_size  = self.vocab_size,
                               seq_len     = args.max_seq_len,
                               d_model     = args.hidden,
                               n_layers    = args.n_layers,
                               n_heads     = args.n_attn_heads,
