...
"""
python -i fns_dynamics.py --models_root=
"""
...
if __name__ == '__main__':
    ...
    parser.add_argument('--is_attn_w', type=str2bool, nargs='?', const=True, default=False)
    ...
    for model_idx, model_dir in enumerate(model_dirs):        
        ...
        if model_idx == 0:
            ...
            if dataset_name == 'cifar10':
                ...
                trainloader, testloader, _ = prepare_data(batch_size=batch_size, num_workers=1)
        ...
        for lidx, block in enumerate(model.encoder.blocks):
            ...
            query = x
            value = block.attention.v_projection(x)
            ...
            if not block.attention.qk_share:        
                key = block.attention.k_projection(x)
            ...
            # attention_probs = block.attention.attn_dropout(attention_probs)
        ...
        # # input image        
        # axss[lidx][ax_idx].imshow(transforms.ToPILImage()(X[0]))
        # # recontructed self-attention for images
        # reconstructed_attn_score = np.zeros([X.shape[-2], X.shape[-1]])
        # tracker = np.zeros([X.shape[-2], X.shape[-1]])
        # psize = config['patch_size']

        # K_tilde_flatten = K_tilde[1:,1:].flatten()
        # total_patches = K_tilde_flatten.numel()      
        # patches_per = X.shape[-2] // psize  
        # for patch_idx in range(0, total_patches, psize):
        #     patch_row_start = patch_idx // psize
        #     patch_col_start = patch_idx % psize
        #     reconstructed_attn_score[patch_row_start:patch_row_start+psize, patch_col_start:patch_col_start+psize] =\
        #          K_tilde_flatten[patch_idx].detach().numpy()      
        #     tracker[patch_row_start:patch_row_start+psize, patch_col_start:patch_col_start+psize] = 1
        # # reconstructed attn-score
        # axss[lidx][ax_idx].imshow(reconstructed_attn_score, cmap=cmap_attn, alpha=0.5, norm=cmap_norm)  #     

        # attn-score
        if not args.is_attn_w:
            axss[lidx][ax_idx].imshow(K_tilde.detach().numpy(), cmap=cmap_attn, norm=cmap_norm)  # norm=cmap_norm               
        # attn-weights     
        else:
            axss[lidx][ax_idx].imshow(attention_probs.detach().numpy()[0,0], cmap=cmap_attn)  # norm=cmap_norm 
    ...
    for lidx in range(config['num_hidden_layers']):
        ...
        # axss[lidx][0].set_ylabel('Eigenspectrum')
        # axss[lidx][ncols].set_ylabel('Attn scores')
        # axss[lidx][ncols*2].set_ylabel('Attn weights')

        # for col in range(ncols):
        #     axss[lidx][col + ncols*(nrows-1)].set_xlabel('Token steps')
        # -----------------------
