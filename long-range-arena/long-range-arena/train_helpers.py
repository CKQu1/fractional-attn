...
def get_batch(data, seq_len, in_dim, padding_idx):
    """Get batch from data iterator. Preprocess to padded X,Y form."""
    batch = next(data)
    if len(batch) == 2:
        inputs, targets = batch
        aux_data = {}
    ...
    # Grab lengths from aux if it is there.
    lengths = aux_data.get('lengths', None)
    # Make same sequence length - CHECK that this pads the sequence length to seq_len
    # In fact, need to test the rest of this...
    inputs = [pad(torch.tensor(seq), (0,0,0,seq_len - len(seq)), value=padding_idx) for seq in batch] 
    
    # Inputs is either B x L or B x L x in_dim.
    # If there are not three dimensions and trailing dimension is not equal to in_dim then
    # transform into one-hot.  This should be a fairly reliable fix.
    if (inputs.ndim < 3) and (inputs.shape[-1] != in_dim):
        inputs = one_hot(np.asarray(inputs), in_dim)
